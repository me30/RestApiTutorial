...
import com.commons.restAPI.paggination.CustomPageRequest;
import com.commons.restAPI.service.IFinder;
import com.commons.restAPI.service.IService;
...
public abstract class BasicService<T, R extends JpaRepository<T, Long> & JpaSpecificationExecutor<T>> implements IFinder<T>, IService<T> {
	...
	protected Pageable checkPageable(Pageable pageable) {
		...
		if(pageable.getPageSize()==Integer.MAX_VALUE){
			if(defaultMaxSize>0)
				pageable = PageRequest.of(pageable.getPageNumber(), defaultMaxSize, pageable.getSort());
			else 
